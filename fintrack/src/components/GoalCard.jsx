...
export default function GoalCard({ g }) {
  ...
  return (
    <div className="bg-[#f5f5f3] rounded-xl px-4 py-3.5 mb-2.5">
      <div className="flex justify-between items-start mb-2.5">
        <div className="flex gap-2.5 items-center">
          ...
          <div>
            <p className="text-sm font-medium m-0">{g.name}</p>
            <p className="text-xs text-gray-400 m-0">Target: ${g.target.toLocaleString()}</p>
          </div>
        </div>
        <div className="text-right flex-shrink-0">
          <p className="text-[15px] font-medium m-0 tabular-nums">${g.saved.toLocaleString()}</p>
          <p className="text-xs text-gray-400 m-0">{pct.toFixed(0)}%</p>
        </div>
      </div>
      <div className="h-[7px] bg-gray-200 rounded overflow-hidden mb-1.5">
        <div
      ...
      <div className="flex justify-between">
        ...
        <span className="text-xs font-medium">{projectedDate(g)}</span>
      </div>
    </div>
  );
}
